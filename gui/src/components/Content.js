...
function Content({ view, toggleRefreshNav, selectedTable, semanticLayerInfo, uslNamebyClick, setIsLoading, previewTableName }) {
  ...
  return (
    <div style={{ height: '100%', display: 'flex', flexDirection: 'column', backgroundColor: 'gray' }}>
      {view === 'sqlEditor' && (
        <SqlEditor toggleRefreshNav={toggleRefreshNav} previewTableName={previewTableName} />
      )}
      {view === 'semanticLayer' && (
        <SemanticLayer selectedTable={selectedTable} semanticLayerInfo={semanticLayerInfo} uslNamebyClick={uslNamebyClick} setIsLoading={setIsLoading} previewTableName={previewTableName} />
      )}
